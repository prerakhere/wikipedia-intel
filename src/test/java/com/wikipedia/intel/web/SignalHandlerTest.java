...
class SignalHandlerTest {
    ...
    void addSignal_storesSignalInBuffer() {
        ...
        TrendingSignal signal = new TrendingSignal("Java", 10, now - 300000, now);
    ...
    void ringBuffer_capsAt50Signals_oldestEvicted() {
        ...
        for (int i = 0; i < 55; i++) {
            handler.addSignal(new TrendingSignal("Article-" + i, i + 1, now - 300000, now));
        }
    ...
    void recentSignals_returnsSortedByEditCountDescending() {
        long now = System.currentTimeMillis();
        handler.addSignal(new TrendingSignal("Low", 3, now - 300000, now));
        handler.addSignal(new TrendingSignal("High", 15, now - 300000, now));
        handler.addSignal(new TrendingSignal("Medium", 8, now - 300000, now));
    ...
    void recentSignals_botAnomalySortedByTotalEditCount() {
        ...
        handler.addSignal(new TrendingSignal("Big", 20, now - 300000, now));
    ...
    void recentSignals_excludesSignalsOlderThan10Minutes() {
        ...
        handler.addSignal(new TrendingSignal("Old_Article", 20, elevenMinutesAgo - 300000, elevenMinutesAgo));
        // Recent signal — windowEnd is 5 minutes ago
        handler.addSignal(new TrendingSignal("Recent_Article", 8, fiveMinutesAgo - 300000, fiveMinutesAgo));
    ...
    void recentSignals_includesSignalsExactly10MinutesOld() {
        ...
        handler.addSignal(new TrendingSignal("Boundary_Article", 5, tenMinutesAgo - 300000, tenMinutesAgo));
                ...
                try {
                    for (int i = 0; i < signalsPerThread; i++) {
                        handler.addSignal(new TrendingSignal(
                                "T" + threadId + "-" + i, i + 1, now - 300000, now));
                    }
                } finally {
                    latch.countDown();
                }
    ...
    void handle_getRootPath_returnsHtml() throws IOException {
        long now = System.currentTimeMillis();
        handler.addSignal(new TrendingSignal("TestArticle", 7, now - 300000, now));
