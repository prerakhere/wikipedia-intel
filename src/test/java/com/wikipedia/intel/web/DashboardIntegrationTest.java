...
class DashboardIntegrationTest {
    ...
    void setUp() throws IOException {
        ...
        handler.addSignal(new TrendingSignal("Java_(programming_language)", 12, List.of(), 0, now - 300000, now));
        handler.addSignal(new BotAnomalySignal(18, 20, 0.9, now - 300000, now));
        handler.addSignal(new TrendingSignal("Kafka_(software)", 7, List.of(), 0, now - 300000, now));
    ...
    void getApiSignals_afterAddingMoreSignals_reflectsUpdates() throws Exception {
        ...
        long now = System.currentTimeMillis();
        handler.addSignal(new TrendingSignal("New_Article", 15, List.of(), 0, now - 300000, now));
